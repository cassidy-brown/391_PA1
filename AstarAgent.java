...
public class AstarAgent extends Agent {
    ...
    class MapLocation implements Comparable<MapLocation>
    {
        ...
        //finds the Chebyshev distance from the location to the input goal location
        public void findCheby(MapLocation goal){
        	cheby = Math.max(Math.abs(x - goal.x), Math.abs(y - goal.y));
        	netG = cheby + cost;
        }
        ...
        @Override
        public int hashCode(){
        	Integer xWrap = (Integer)x;
        	return xWrap.hashCode();
        }
        
        @Override
        public int compareTo(MapLocation other){
        	return (int)netG - (int)other.netG;
        }
    }
    ...
    {
    //	if(!currentPath.isEmpty() && state.isUnitAt(currentPath.peek().x, currentPath.peek().y))
    	//	return true;
    	//else
            return false;
    }
    ...
    {
    	goal = new MapLocation(goal.x, goal.y, null, 0);
        MapLocation current = start = new MapLocation(start.x, start.y, null, 0);
        System.out.println("Start:(" + start.x + ", " + start.y + ")");
        ArrayList<MapLocation> openList = new ArrayList<MapLocation>();
        openList.add(current);
        
        while(!openList.isEmpty() && !openList.contains(goal)){
        	System.out.println("Current:(" + current.x + ", " + current.y + ") openSize?" + openList.size());
        	if(!resourceLocations.contains(current)){
        		getValidNeighbors(current, resourceLocations, openList, goal, xExtent, yExtent); 
        		openList.remove(current);
        		resourceLocations.add(current);
        		//Print contents of openList
        	/*	Object[] list = openList.toArray();
        		for(Object o : list){
        			MapLocation oMap = (MapLocation)o;
        			System.out.print("(" + oMap.x + ", " + oMap.y + ") ");
        			//System.out.print(oMap.cost + " ");
        		}
        		System.out.println(); */
        	}
        ...
        //End condition: path to goal found, or there is no available path
        int goalIndex = openList.indexOf(goal);
        if(goalIndex == -1){					//if no path to goal
        	System.out.println("No available path");
        	return new Stack<MapLocation>();
        } else {			//else there exists a path to the goal
        	goal = openList.get(goalIndex);
        	System.out.println("GOAL loc:(" + goal.x + ", " + goal.y + ") cameFrom:(" + goal.cameFrom.x + ", " + goal.cameFrom.y + ") cost:" + goal.cost);
        	MapLocation backtrack = goal;
        	Stack<MapLocation> path = new Stack<MapLocation>();
        	while(backtrack.cameFrom != start){
        		//System.out.println("pathbuild loop; backtrack:(" + backtrack.x + ", " + backtrack.y +") cameFrom:(" + backtrack.cameFrom);
        		path.push(backtrack.cameFrom);
        		backtrack = backtrack.cameFrom;
        	}	
        	//Prints the path from goal to start (order of Stack's array)
        	Object[] pathArr = path.toArray();
        	for(Object o : pathArr){
        		MapLocation point = (MapLocation)o;
    ...
    //helper method for AstarSearch which adds the necessary neighbors of the input location to the given open list 
    private void getValidNeighbors(MapLocation current, Set<MapLocation> closedList, ArrayList<MapLocation> openList, MapLocation goal, int xExtent, int yExtent){
    	MapLocation neighbor;	
    	if(current.x - 1 >= 0){
    		neighbor = new MapLocation(current.x - 1, current.y - 1, current, current.cost + 1);
    		if(current.y - 1 >= 0 && !closedList.contains(neighbor) && !openList.contains(neighbor)){
    			neighbor.findCheby(goal);
    			openList.add(neighbor);
    		}
    		neighbor = new MapLocation(current.x - 1, current.y, current, current.cost + 1);
    		if(!closedList.contains(neighbor) && !openList.contains(neighbor)){
    			neighbor.findCheby(goal);
    			openList.add(neighbor);
    		}
    		neighbor = new MapLocation(current.x - 1, current.y + 1, current, current.cost + 1);
    		if(current.y + 1 < yExtent && !closedList.contains(neighbor) && !openList.contains(neighbor)){
    			neighbor.findCheby(goal);
    			openList.add(neighbor);
    		}			
    	}
    	if(current.x + 1 < xExtent){
    		neighbor = new MapLocation(current.x + 1, current.y - 1, current, current.cost + 1);
    		if(current.y - 1 >= 0 && !closedList.contains(neighbor) && !openList.contains(neighbor)){
    			neighbor.findCheby(goal);
    			openList.add(neighbor);
    	...
    	neighbor = new MapLocation(current.x, current.y - 1, current, current.cost + 1);
    	if(current.y - 1 >= 0 && !closedList.contains(neighbor) && !openList.contains(neighbor)){
			neighbor.findCheby(goal);
			openList.add(neighbor);
		}
